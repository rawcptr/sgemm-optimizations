...
template<std::size_t SIZE_M, std::size_t SIZE_N>
using Tile = float[SIZE_M][SIZE_N];
...
template<std::size_t TILE_M, std::size_t TILE_N>
__device__ inline void smem_store(
  Tile<TILE_M, TILE_N>& smem, std::size_t row, std::size_t col, float4 v
) {
  smem[row][col + 0] = v.x;
  smem[row][col + 1] = v.y;
  smem[row][col + 2] = v.z;
...
template<std::size_t TILE_M, std::size_t TILE_N, std::size_t TM, std::size_t TN>
__device__ inline void mma_tile(
  float (&acc)[TM][TN],
  const Tile<TILE_M, TILE_N>& As,
  const Tile<TILE_M, TILE_N>& Bs,
  std::size_t ty,
  std::size_t tx
) {
  // #pragma unroll
...
__global__ void sgemm_v5(
  ...
  static_assert(WM * WN == 32 * TM * TN, "Warp tile must map exactly to 32 threads");
